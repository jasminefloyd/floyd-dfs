...
import type { CandidateType, LineupCandidate, ObjectiveProfile, OptimizerInput, OptimizerOptions, OptimizerPackage, OptimizerService, DuplicationRisk } from "@sports-engine/contracts";
...
const SLOT_ORDER = (slots: Record<string, { count: number }>): string[] => Object.entries(slots).flatMap(([slot, rule]) => Array.from({ length: rule.count }, (_, index) => rule.count > 1 ? `${slot}_${index + 1}` : slot));

export class DeterministicOptimizer implements OptimizerService {
  optimize(input: OptimizerInput, options: OptimizerOptions = {}, now = new Date()): OptimizerPackage {
    const contestSize = input.validatedSlate.contest.contestSize;
    const profile = options.objectiveProfile ?? objectiveProfileForContest(contestSize);
    ...
    if (input.projectionPackage.status === "BLOCKED") return blocked(input, profile, now, ["ProjectionPackage is BLOCKED; no legal lineup can be evaluated."]);
    const projectionByPlayer = new Map(input.projectionPackage.players.map((player) => [player.playerId, player]));
    const slots = SLOT_ORDER(input.validatedSlate.rosterRules.slots);
    if (!slots.length) return blocked(input, profile, now, ["No roster slots are available."]);
    ...
    if (!generated.length) return blocked(input, profile, now, ["No legal lineups satisfy roster eligibility, salary cap, and team constraints."]);
    const raw = generated.map((lineup) => scoreCandidate(lineup, input, projectionByPlayer, profile));
    const ranked = rankCandidates(raw, maxCandidates);
    assignTypes(ranked);
...
function objectiveProfileForContest(contestSize: number | undefined): ObjectiveProfile {
...
function enumerate(slots: string[], index: number, rosterSlots: Record<string, string>, salaryUsed: number, used: Set<string>, input: OptimizerInput, output: Array<{ rosterSlots: Record<string, string>; salaryUsed: number }>, limit: number): void {
  ...
  for (const player of input.validatedSlate.playerPool) {
    ...
    if (maxPerTeam && team && Object.values(rosterSlots).map((id) => input.validatedSlate.playerPool.find((candidate) => candidate.playerId === id)?.team).filter((value) => value === team).length >= maxPerTeam) continue;
    rosterSlots[slot] = player.playerId; used.add(player.playerId);
    enumerate(slots, index + 1, rosterSlots, salaryUsed + salary, used, input, output, limit);
    delete rosterSlots[slot]; used.delete(player.playerId);
  }
}
...
function scoreCandidate(lineup: { rosterSlots: Record<string, string>; salaryUsed: number }, input: OptimizerInput, projectionByPlayer: Map<string, OptimizerInput["projectionPackage"]["players"][number]>, profile: ObjectiveProfile): LineupCandidate {
  ...
  const floor = Object.entries(lineup.rosterSlots).reduce((sum, [slot, id]) => sum + scaledProjection(projectionByPlayer.get(id)?.projectedOutcomes.floorP20 ?? 0, input.validatedSlate.rosterRules.slots[baseSlot(slot)]?.fantasyMultiplier), 0);
  const median = Object.entries(lineup.rosterSlots).reduce((sum, [slot, id]) => sum + scaledProjection(projectionByPlayer.get(id)?.projectedOutcomes.medianP50 ?? 0, input.validatedSlate.rosterRules.slots[baseSlot(slot)]?.fantasyMultiplier), 0);
  const ceiling = Object.entries(lineup.rosterSlots).reduce((sum, [slot, id]) => sum + scaledProjection(projectionByPlayer.get(id)?.projectedOutcomes.ceilingP90 ?? 0, input.validatedSlate.rosterRules.slots[baseSlot(slot)]?.fantasyMultiplier), 0);
  ...
  const teamCounts = new Map<string, number>(); for (const player of playerRows) if (player.team) teamCounts.set(player.team, (teamCounts.get(player.team) ?? 0) + 1);
...
function blocked(input: OptimizerInput, profile: ObjectiveProfile, now: Date, gaps: string[]): OptimizerPackage { return { slateId: input.validatedSlate.slateId, tenantId: input.validatedSlate.tenantId, sport: input.validatedSlate.sport, version: 1, generatedAt: now.toISOString(), objectiveProfile: profile, candidates: [], warnings: [], gaps, status: "BLOCKED" }; }

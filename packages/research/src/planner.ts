import type { ResearchPlan, ResearchQuestion, ResearchBucket, ValidatedSlate } from "@sports-engine/contracts";

const BASE_QUESTIONS: Array<[ResearchBucket, string, ResearchQuestion["priority"], number[], number]> = [
...
export function createResearchPlan(slate: ValidatedSlate, now = new Date()): ResearchPlan {
  const questions: ResearchQuestion[] = BASE_QUESTIONS.map(([bucket, question, priority, tiers, freshness], index) => ({
    id: `${slate.slateId}:${bucket.toLowerCase()}`,
    bucket,
    question: `${slate.sport} ${slate.league}: ${question}`,
    priority,
    preferredSourceTiers: tiers as ResearchQuestion["preferredSourceTiers"],

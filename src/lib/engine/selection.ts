import type { LineupCandidate, ResearchPackage, SelectedLineup, SelectionPackage, ValidatedSlate } from './contracts.js';
...
function rankForContext(candidates: LineupCandidate[], contestSize: number | undefined, entries: number): LineupCandidate[] {
  const tournament = (contestSize ?? 0) >= 10_000 || entries > 1;

...
async function requestWithRetry(fetcher: typeof fetch, url: string, init: RequestInit, signal?: AbortSignal): Promise<Response> {
  const maxAttempts = 3;
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const response = await fetcher(url, { ...init, signal });
    if (response.ok || attempt === maxAttempts || (response.status !== 429 && response.status < 500)) return response;
    const retryAfter = Number(response.headers.get('retry-after'));
    const delayMs = Number.isFinite(retryAfter) && retryAfter >= 0 ? Math.min(retryAfter * 1000, 5000) : 250 * 2 ** (attempt - 1);
    await delay(delayMs, signal);
  }
  throw new Error('OpenAI research request exhausted its retry attempts.');
}

function delay(milliseconds: number, signal?: AbortSignal): Promise<void> { return new Promise((resolve, reject) => { const timer = setTimeout(resolve, milliseconds); if (signal) signal.addEventListener('abort', () => { clearTimeout(timer); reject(signal.reason ?? new Error('Request aborted.')); }, { once: true }); }); }

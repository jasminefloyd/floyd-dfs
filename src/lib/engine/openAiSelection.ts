...
export async function selectWithOpenAi(input: { slate: ValidatedSlate; research: ResearchPackage; candidates: LineupCandidate[]; selection: SelectionPackage; cashLineCalibration?: CashLineCalibration }, options: OpenAiSelectionOptions): Promise<SelectionPackage> {
  ...
  const response = await requestWithRetry(fetcher, options.endpoint ?? 'https://api.openai.com/v1/responses', { method: 'POST', headers: { authorization: `Bearer ${options.apiKey}`, 'content-type': 'application/json' }, body: JSON.stringify({
...
async function requestWithRetry(fetcher: typeof fetch, url: string, init: RequestInit): Promise<Response> { for (let attempt = 1; attempt <= 3; attempt += 1) { const response = await fetcher(url, init); if (response.ok || attempt === 3 || (response.status !== 429 && response.status < 500)) return response; const retryAfter = Number(response.headers.get('retry-after')); await new Promise((resolve) => setTimeout(resolve, Number.isFinite(retryAfter) && retryAfter >= 0 ? Math.min(retryAfter * 1000, 5000) : 250 * 2 ** (attempt - 1))); } throw new Error('OpenAI Selection request exhausted its retry attempts.'); }

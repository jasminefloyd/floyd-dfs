...
export function assertOptimizer(value: OptimizerPackage, slate: ValidatedSlate): void { const errors: string[] = []; const ids = new Set(slate.playerPool.map((player) => player.playerId)); for (const candidate of value.candidates) { if (candidate.playerIds.some((id) => !ids.has(id))) errors.push(`candidate ${candidate.id} references a player outside the slate.`); if (candidate.salaryUsed > slate.salaryCap) errors.push(`candidate ${candidate.id} exceeds salary cap.`); if (slate.rosterRules.uniquePlayersRequired && new Set(candidate.playerIds).size !== candidate.playerIds.length) errors.push(`candidate ${candidate.id} rosters a duplicate player.`); if (Object.keys(candidate.rosterSlots).length !== Object.values(slate.rosterRules.slots).reduce((sum, slot) => sum + slot.count, 0)) errors.push(`candidate ${candidate.id} does not fill every required roster slot.`); } if (errors.length) throw new EngineContractError('OPTIMIZE', errors); }

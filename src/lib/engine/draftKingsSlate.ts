...
function readDraftStatFppg(record: Record<string, unknown>): number | undefined { const attributes = Array.isArray(record.draftStatAttributes) ? record.draftStatAttributes : []; const fppg = attributes.find((value) => { const attribute = asRecord(value); return String(attribute?.id ?? '') === '90'; }); return readNumber(asRecord(fppg) ?? {}, ['value', 'sortValue']); }

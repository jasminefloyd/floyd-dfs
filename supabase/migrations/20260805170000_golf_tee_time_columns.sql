-- Golf wave-correlation needs each golfer's round-1 tee time, which the live DK slate
-- JSON already carries end-to-end into the lineup generator. This migration adds the
-- same columns to draftkings_player_salaries and its manual-import RPC so the stored/
-- fallback read path (fantasy_ai_get_draftkings_salaries) stays consistent with the
-- live-slate path for golf.

ALTER TABLE tenant_fantasy_ai.draftkings_player_salaries
  ADD COLUMN IF NOT EXISTS tee_time TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS starting_hole TEXT;
...
BEGIN
  ...
  SELECT
    ...
    CASE
      WHEN NULLIF(row_data->>'tee_time', '') IS NOT NULL
      THEN (row_data->>'tee_time')::TIMESTAMPTZ
      ELSE NULL
    END,
    NULLIF(row_data->>'starting_hole', ''),
    NOW()
...
DROP FUNCTION IF EXISTS public.fantasy_ai_get_draftkings_salaries(TEXT, DATE, TEXT, TEXT);
